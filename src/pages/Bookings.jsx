...
export default function Bookings() {
  const { trips, driversById , newRideQueue } = useOps()
...
<article className="panel overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100">
          <h3 className="font-semibold text-slate-900">New Rides Queue</h3>
          <p className="text-xs text-slate-500 mt-1">Incoming ride requests waiting for manual assignment.</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {newRideQueue.length === 0 && (
                <tr>
                  <td className="px-4 py-4 text-slate-500" colSpan={8}>No rides are waiting for assignment.</td>
                </tr>
              )}
              {newRideQueue.map((trip) => (
                <tr
                  key={trip.id}
                ...
                >
                  ...
                  <td className="px-4 py-3 text-slate-700">{trip.customer.name}</td>
                  <td className="px-4 py-3 text-slate-700">{trip.pickupAddress}</td>
                  <td className="px-4 py-3 text-slate-700">{trip.dropoffAddress}</td>
                  <td className="px-4 py-3 text-slate-700">{trip.estimatedDurationMin} min</td>
                  <td className="px-4 py-3 text-slate-700">{trip.estimatedDistanceMiles} mi</td>
                  <td className="px-4 py-3"><span className="px-2 py-1 rounded-full bg-amber-100 text-amber-700 text-xs font-semibold">{statusLabel(trip.status)}</span></td>
